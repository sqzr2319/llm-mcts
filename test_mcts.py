...
def main():
    ...
    try:
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            for item in per_item_result:
                f.write(f"Problem {item['index']:03d}: ratio={item['ratio_correct_terminal']:.6f} gv={item['gv_path']}\n")
            f.write(f"\nAverage ratio over {len(dataset)} problems: {avg_ratio:.6f}\n")
